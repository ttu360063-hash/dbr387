...
export default function ManageCommunity() {
  const { community } = useStore((state) => state.data);
  ...
  const handleSave = (e) => {
    ...
    toast.success('Comunidade salva com sucesso!');
  };

  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-black mb-2">Comunidade</h1>
        <p className="text-gray-400">Edite as informações da área do Discord.</p>
      </div>
        ...
        <div className="bg-[#111] border border-white/10 rounded-xl p-6">
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-400 mb-2">Link do Discord</label>
                <input type="text" value={data.url} onChange={e => setData({...data, url: e.target.value})} className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-red-500" />
              </div>
            </div>
          </div>
        </div>
